 ...
 */
angular.module('borrow_memo').controller('MainActivityCtrl', ['$scope', 'AuthService', 'DataBaseService', 'ngDialog', 'BorrowedItems', '$rootScope', '$location', 'SelectedCity',
    function MainActivityController($scope, AuthService, DataBaseService, ngDialog, BorrowedItems, $rootScope, $location, SelectedCity) {
        $scope.postData={};
        ...
        $scope.saveItem = function(postData){
            if (postData.location==undefined || postData.name==undefined ||
                ...
                DataBaseService.saveItem($scope.postData).then(function(data){
                        if (selectedCity==postData.location) {
